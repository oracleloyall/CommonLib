...
ncx_slab_free_locked(ncx_slab_pool_t *pool, void *p)
{
    ...
    printf("slab free: %p", p);

    if ((u_char *) p < pool->start || (u_char *) p > pool->end) {
        perror("ncx_slab_free(): outside of pool");
        goto fail;
    }
    ...
    case NCX_SLAB_PAGE:
		...
		if (slab == NCX_SLAB_PAGE_FREE) {
			printf("ncx_slab_free(): page is already free");
			goto fail;
        }

		if (slab == NCX_SLAB_PAGE_BUSY) {
			printf("ncx_slab_free(): pointer to wrong page");
			goto fail;
        }
...
wrong_chunk:
	...
	perror("ncx_slab_free(): pointer to wrong chunk");

    goto fail;

chunk_already_free:

	perror("ncx_slab_free(): chunk is already free");
...
ncx_slab_alloc_pages(ncx_slab_pool_t *pool, ncx_uint_t pages)
{
    ...
    perror("ncx_slab_alloc() failed: no memory");
