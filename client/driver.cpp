...
void on_white_list(string wl)
{
	...
	{
		logout("[on_white_list]sock 成功\n");
	}
	char request[ 1024 ] = { 0 };
	sprintf(request, "whitelist %s\r\n\r\n", wl.c_str());
	int m = send_request(sock, request);
	logout("[on_white_list]send 白名单命令：%s\n",request );
...
//	printf("end_ip=%s\n", end_ip.c_str());
	...
	{
		if (lan->dhcp_mode == 0)
			MyExec("uci set dhcp.lan.ignore=1");
		else
			MyExec("uci set dhcp.lan.ignore=0");
	}
	//修改leasetime
	if (0 != lan->leasetime)
	{
...
//			printf("当前uci命令为设置dhcp limit：%s\n",buf);
	...
	//MyExec("uci commit dhcp");
	//MyExec("uci commit network");
...
static int on_ap_wlan_conf(pwlan_entity2 wlan)
{
	logout("coming into wlan config\n");
		...
		//	unsigned char cWlanSsid2[ 255 ] = { 0 };
		   ...
		   {
			    bzero( cCmd, sizeof( cCmd ) );
			    sprintf( (char *)cCmd, "uci set wireless.@wifi-iface[%d].disabled=0",iValue );
		   ...
		   {
			   bzero( cCmd, sizeof( cCmd ) );
			   sprintf( (char *)cCmd, "uci set wireless.@wifi-iface[%d].disabled=1",iValue );
		   ...
		   {
			   bzero( cCmd, sizeof( cCmd ) );
			   sprintf( (char *)cCmd, "uci set wireless.@wifi-device[%d].txpower=%d", iValue, wlan->wlan_txpower );
	...
	//		   sprintf( (char *)cCmd, "uci set wireless.@wifi-device[0].channel=%d", wlan->wlan_channel );
...
#endif
		if( strlen( (char *)wlan->secret_key ) >= 8 )
		{
			...
			else if( wlan->sofe_mode == 3 )  //wpa
			{
				...
				{
					bzero( cCmd, sizeof( cCmd ) );
					sprintf( (char *)cCmd, "uci set wireless.@wifi-iface[%d].encryption=psk+ccmp",iValue );
					MyExec( ( char * )cCmd );
				}
		...
		else
		{
	      ...
	      if( wlan->session_limit > 0 )
	      {
			...
			//MyExec("uci commit dhcp");
			//MyExec("uci commit wireless");
			//MyExec("/etc/init.d/wireless restart");
			//MyExec("uci commit wireless");
			return( 1 );
}
...
void on_ap_runtime_conf(pap_net_conf2 body)										// 配置配置相关
{
	...
	MyExec("/etc/init.d/wireless restart");
	MyExec("/etc/init.d/dhcp restart");
...
void ap_usraction(usr_action* action)
{
	...
	__u32 *pip = (__u32*)&local_ip.sin_addr;
	__u32 *pmask = (__u32*)&local_mask.sin_addr;
	memset(&local_ip,0,sizeof(local_ip));
	memset(&local_mask,0,sizeof(local_mask));
	GetSysMaskBySocket(ec->Option.Other.interface.c_str(),pmask);
	GetSysIpBySocket(ec->Option.Other.interface.c_str(),pip);
	...
	char dst[32] = "";
	char mac[6] = "";
    logout("local_mask.sin_addr.s_addr==%d\n",local_ip.sin_addr.s_addr);
    logout("--%d : %d \n",*(__u32*)&(action->src.addr) & *pmask,( *pip & *pmask));
