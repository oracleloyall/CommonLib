...
Ipc::MemMap::MemMap(const char * const aPath) :
		cleaner(NULL), path(aPath), shared(shm_old(Shared)(aPath))
{
...
Ipc::MemMap::openForWritingAt(const sfileno fileno, bool overwriteExisting)
{
	...
	if (lock.lockExclusive()) {
		...
		if (!s.waitingToBeFreed && !s.empty() && !overwriteExisting) {
			...
			debugs(54, 5,
					"cannot open existing entry " << fileno << " for writing "
							<< path);
			return NULL;
		}
		...
		debugs(54, 5,
				"opened slot at " << fileno << " for writing in map [" << path
						<< ']');
		return &s; // and keep the entry locked
	}

	debugs(54, 5,
			"failed to open slot at " << fileno << " for writing in map ["
					<< path << ']');
	return NULL;
}
...
{
	debugs(54, 5, "abort writing slot at " << fileno <<lockExclusive
			" in map [" << path << ']');
	assert(valid(fileno));
	Slot &s = shared->slots[fileno];
	assert(s.writing());
...
Ipc::MemMap::openForReading(const cache_key * const key, sfileno &fileno)
{
	...
	if (const Slot *slot = openForReadingAt(idx)) {
		if (slot->sameKey(key)) {
			fileno = idx;
			debugs(54, 5,
					"opened slot at " << fileno << " for key "
							<< storeKeyText(key) << " for reading in map ["
							<< path << ']');
...
static
unsigned int hash_key(const unsigned char *data, unsigned int len,
...
void Ipc::MemMap::freeLocked(Slot &s, bool keepLocked)
{
	if (!s.empty() && cleaner)
	...
	--shared->count;
	debugs(54, 5,
			"freed slot at " << (&s - shared->slots.raw()) << " in map ["
					<< path << ']');
}
...
void
 Ipc::MemMapSlot::set(const unsigned char *aKey, const void *block,
		size_t blockSize, time_t expireAt)
{
...
size_t
 Ipc::MemMap::Shared::SharedMemorySize(const int limit,
		const size_t extrasSize)
{
