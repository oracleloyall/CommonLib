...
udp_peer_t* udp_peer_new(loop_t *loop, const char *ip,  on_message_f messagecb, on_writable_f writecb, void *userdata)
{
    ...
    if (NULL == loop || NULL == ip ||NULL == messagecb)
    {
        log_error("udp_peer_new: bad loop(%p) or bad ip(%p)  or bad messagecb(%p)", loop, ip,  messagecb);
        return NULL;
    }

    fd = create_udp_socket(ip);
    if (fd < 0)
    {
        log_error("udp_peer_new: create_udp_socket() failed, local addr: %s, errno: %d", ip,  errno);
        return NULL;
    }
...
int udp_peer_send2(udp_peer_t* peer, const void *message, unsigned len, const struct sockaddr_in *peer_addr)
{
    ...
    ret = 0;
    result = sendto(peer->fd, message, len, 0, ( struct sockaddr*)peer_addr, sizeof(*peer_addr));
    if (len != result)
    {
