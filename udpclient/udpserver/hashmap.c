...
*/
map_t hashmap_new(int initSize){
...
//return MAP_OMEM, or MAP_OK
int hashmap_put(map_t in, char* key, any_t value){
    int index;
    hashmap_map* m = (hashmap_map *)in;
    //find place to put value
    index = hashmap_hash(in, key);//if MAP_FULL rehash the table
    int numRe = 0;
    while(index == MAP_FULL){
        if(numRe > 5) return MAP_OMEM;//check if rehash too many time
...
//Lay gia tri lu trong hashmap voi 1 key
int hashmap_get(map_t in, char* key, any_t* arg){
    ...
    for(i = 0; i < MAX_CHAIN_LENGTH; i++){
        ...
        if(in_use == 1){
            if(strcmp(m->data[curr].key, key) == 0){//neu tim thay phan tu co key dung
                *arg = (m->data[curr].data);
                //printf("hash get intoMAP_OK \n");
                return MAP_OK;
            }
        }
        curr = (curr + 1) % m->table_size;
    ...
    *arg = NULL;
   // printf("hash get into MAP_MISSING\n");
    return MAP_MISSING;
}
